...
describe('runInit', () => {
  ...
  it('creates the full .recall directory structure', async () => {
    ...
    await expect(access(join(dir, '.recall', 'snapshots', 'latest.json'))).resolves.toBeUndefined();
    for (const file of [
      'architecture.md',
      'conventions.md',
      'decisions.md',
  ...
  it('adds .recall/cache/ to .gitignore', async () => {
  ...
  it('preserves manually edited memory file content across a second init', async () => {
    await runInit({ path: dir, toolVersion: '0.1.0' });
