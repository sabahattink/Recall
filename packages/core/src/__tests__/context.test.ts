...
describe('runContext', () => {
  ...
  it('writes context.md by default', async () => {
  ...
  it('does not write a file when stdout mode is requested', async () => {
    await runInit({ path: dir, toolVersion: '0.1.0' });
  ...
  it('produces task-focused output that differs from the default', async () => {
    await runInit({ path: dir, toolVersion: '0.1.0' });
